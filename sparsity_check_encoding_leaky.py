...
def main():
    ...
    model = DispNetS_Q_full_encoding_leaky(type = args.dtype, n_exp_en = args.exp_en, n_man_en =args.mant_en, n_exp_de = args.exp_de , n_man_de = args.mant_de, slice_width = args.slice_width ,mode = args.mode, device = "cuda" )
            ...
            if isinstance(child, nn.Sequential):
                for _, sub_child in child.named_children():
                    if isinstance(sub_child, nn.Conv2d):
                        ...
                        total_count_weight += encoding(bin_fxp(sub_child_quant_weight,args.exp_en, args.mant_en, args.mode, device), args.exp_en+args.mant_en, args.slice_width)[0]
                        zero_count_weight += encoding(bin_fxp(sub_child_quant_weight,args.exp_en, args.mant_en, args.mode, device), args.exp_en+args.mant_en, args.slice_width)[1]
                        
                        total_count_weight += encoding(bin_fxp(sub_child_quant_bias,args.exp_en, args.mant_en, args.mode, device), args.exp_en+args.mant_en, args.slice_width)[0]
                        zero_count_weight += encoding(bin_fxp(sub_child_quant_bias,args.exp_en, args.mant_en, args.mode, device), args.exp_en+args.mant_en, args.slice_width)[1]
            ...
            if isinstance(child, nn.Sequential):
                for _, sub_child in child.named_children():
                    if isinstance(sub_child, nn.Conv2d):
                        ...
                        total_count_weight += encoding(bin_fxp(sub_child_quant_weight,args.exp_de, args.mant_de, args.mode, device), args.exp_de+args.mant_de, args.slice_width)[0]
                        zero_count_weight += encoding(bin_fxp(sub_child_quant_weight,args.exp_de, args.mant_de, args.mode, device), args.exp_de+args.mant_de, args.slice_width)[1]
                        
                        total_count_weight += encoding(bin_fxp(sub_child_quant_bias,args.exp_de, args.mant_de, args.mode, device), args.exp_de+args.mant_de, args.slice_width)[0]
                        zero_count_weight += encoding(bin_fxp(sub_child_quant_bias,args.exp_de, args.mant_de, args.mode, device), args.exp_de+args.mant_de, args.slice_width)[1]
                        sub_child.weight.data = sub_child_quant_weight.to(device)
                        sub_child.bias.data = sub_child_quant_bias.to(device)
